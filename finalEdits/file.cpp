...
public:
    ...
    {
        out << "room number: " << room.number << ", total days of reservation: " << room.totalDays << '\n';
...
public:
    ...
    {
        //pravim txt fail v koito da zapisvame datite na koito sa bili zaeti staite koito sega osvobojdavame.
        ofstream listOfPreviousBusyRooms("PreviousBusyRooms.txt",ios::app);

        if (!listOfPreviousBusyRooms.is_open())
        {
            cout << "Error" << endl;
        }
        ...
        for(int i = 0; i < totalApps; i++)
        {
            if (_number == list[i].getNumber())
            {
                char startDate_[30];
                char endDate_[30];
                strcpy(startDate_, list[i].getStartDate());
                strcpy(endDate_, list[i].getEndDate());
                //zapisvame datite na staqta koqto shte osvobodim.
                listOfPreviousBusyRooms << list[i].getNumber() << " " << startDate_ << " " << endDate_ << endl;
                //
                list[i].setisBusy(0);
                list[i].setName("");
                list[i].setstartDate("");
    ...
    void IsItBusyForThisPeriod(const char _startDate[],const char _endDate[])
    {
        ...
        //prevrushtame startovite dati i krainite dati na zadadeniq ot NAS period
        ...
        //PROVERKA DALI NQKOQ OT STAITE KOITO SA BILI ZAETI SAVPADA S DADENIQ PERIOD
        ifstream pastRooms("PreviousBusyRooms.txt");

        while (!pastRooms.eof())
        {
                char tempLine[1000];
                pastRooms.getline(tempLine, 1000);
                char roomNumber[50];
                //vzimame nomera na staqta;
                int lastI = 0;
                for (int i = 0; i < 100; i++)
                {
                    if (tempLine[i] == ' ' || tempLine[i] == '\0')
                    {
                        lastI = i;
                        roomNumber[i] = '\0';
                        break;
                    ...
                    roomNumber[i] = tempLine[i];
                }
                //vzimane na startdate i enddate;
                char preroomstartDate[50];
                char preroomsendDate[50];

                int lastJ = 0;
                int _temp1 = 0;
                int _temp2 = 0;
                for (int j = lastI+1; j < 1000; j++)
                {
                    if (tempLine[j] == ' ' || tempLine[j] == '\0')
                    {
                        lastJ = j;
                        preroomstartDate[_temp1] = '\0';
                        break;

                    }
                    preroomstartDate[_temp1] = tempLine[j];
                    _temp1++;
                }
                for (int z = lastJ + 1; z < 1000; z++)
                {
                    if (tempLine[z] == ' ' || tempLine[z] == '\0')
                    {
                        preroomsendDate[_temp2] = '\0';

                        break;

                    }
                    preroomsendDate[_temp2] = tempLine[z];
                    _temp2++;
                }
                int currsyears = (preroomstartDate[0] - 48) * 1000 + (preroomstartDate[1] - 48) * 100 + (preroomstartDate[2] - 48) * 10 + (preroomstartDate[3] - 48);
                int currsmonths = ((preroomstartDate[4] - 48) * 10 + preroomstartDate[5] - 48);
                int currsdays = ((preroomstartDate[6] - 48) * 10 + preroomstartDate[7] - 48);
                int curreyears = (preroomsendDate[0] - 48) * 1000 + (preroomsendDate[1] - 48) * 100 + (preroomsendDate[2] - 48) * 10 + (preroomsendDate[3] - 48);
                int curremonths = ((preroomsendDate[4] - 48) * 10 + preroomsendDate[5] - 48);
                int curredays = ((preroomsendDate[6] - 48) * 10 + preroomsendDate[7] - 48);
        ...
        // PROVERKA DALI NQKOQ OT V MOMENTA ZAETITE STAI NE E V TOZI PERIOD
        for (int i = 0; i < totalApps; i++)
        {
            //prevrushtame startovite i krainite dati na konkretniq period na dadena staq ot masiva sus vsichki stai
   ...
   if (!list.is_open())
    ...
    while (true)
    {
        ...
        if (strcmp(command, "EXIT") == 0)
        {
            ...
            ofstream listOfPreviousBusyRooms("PreviousBusyRooms.txt");
            listOfPreviousBusyRooms.clear();
            listOfPreviousBusyRooms.close();
            remove("PreviousBusyRooms.txt");
            break;
        }
